...
import {
    ...
    FileText, Sheet, Github, ArrowDown, Maximize2, Download // Added new icons
} from 'lucide-react';
...
class GoogleSheetsService {
    ...
    async getLatestData(): Promise<RealtimeEnergyData | null> {
        try {
            ...
            const response = await fetch(
                `https://sheets.googleapis.com/v4/spreadsheets/${this.sheetId}/values/Sheet1!A:K?key=${this.apiKey}&majorDimension=ROWS&valueRenderOption=UNFORMATTED_VALUE`
            );
    ...
    async getHistoricalData(selectedDate: string): Promise<EnergyDataRow[]> {
        try {
            ...
            const response = await fetch(
                `https://sheets.googleapis.com/v4/spreadsheets/${this.sheetId}/values/Sheet1!A:K?key=${this.apiKey}`
            );
    ...
    private parseRowToEnergyData(row: any[]): EnergyDataRow {
        return {
            ...
            'DutyCycle_%_24H': parseFloat(row[11]) || 0,
            Cycle_ID: 0
        };
    }
...
const useRealtimeData = () => {
    ...
    useEffect(() => {
        fetchLatestData();
        const interval = setInterval(fetchLatestData, 30000); // Every 30 seconds
...
export default function App() {
    ...
    const displayData = useMemo(() => {
        if (viewMode === 'realtime') {
            return currentData ? [currentData] : (energyData.length > 0 ? [energyData[energyData.length - 1]] : []);
        } else {
            return historicalData.length > 0 ? historicalData : energyData;
        }
    }, [viewMode, currentData, historicalData, energyData]);
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-gray-200 font-sans">
            <DashboardLayout 
                ...
                onChartEnlarge={setEnlargedChart}
            />
...
const DashboardLayout = ({ 
    ...
    const [activeTab, setActiveTab] = useState('dashboard'); // Changed from 'introduction'
            ...
            <div className="flex-1 flex flex-col h-screen">
                ...
                <main className={`flex-1 overflow-y-auto ${activeTab !== 'introduction' ? 'p-8' : ''} bg-slate-900`}>
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'dashboard' && <DashboardPage key="dashboard" data={data} onChartEnlarge={onChartEnlarge} />}
...
const Sidebar = ({ activeTab, setActiveTab, isOnline }: { 
    ...
    return (
        ...
        >
            ...
            <div className={`mt-auto p-4 rounded-xl border ${isOnline ? 'bg-slate-700/50 border-green-500/30' : 'bg-slate-700/50 border-red-500/30'}`}>
                ...
                <div className="flex items-center mt-2 text-xs">
                    ...
                    <span className={isOnline ? 'text-green-400' : 'text-red-400'}>
                        Signal: {isOnline ? 'Strong' : 'Disconnected'}
                    </span>
                </div>
            </div>
        </motion.nav>
...
// --- Enhanced Dashboard Page with Chart Enlargement ---
const DashboardPage = ({ data, onChartEnlarge }: { 
    data: EnergyDataRow[], 
    onChartEnlarge: (chart: {type: string, title: string}) => void 
}) => {
            ...
            <div className="space-y-8">
                <AnimatedView>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <MetricCard title="Total Energy Consumed" value={latest.Energy_kWh || 0} unit="kWh" icon={Zap} />
                        <MetricCard title="Cumulative Cost" value={latest.Cost_cum_BDT || 0} unit="BDT" icon={DollarSign} />
                        <MetricCard title="Peak Power" value={data.length > 0 ? Math.max(...data.map(d => d.ActivePower_kW)) : 0} unit="kW" icon={TrendingUp} />
                        <MetricCard title="Average Power Factor" value={data.length > 0 ? data.reduce((a,b)=>a+b.PowerFactor,0)/data.length : 0} unit="" icon={Activity} />
                    </div>
                </AnimatedView>
                 <AnimatedView>
...
// --- Chart Components with Click-to-Enlarge ---
